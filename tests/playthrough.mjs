...
let lastGoal = -1, deaths = 0, frames = 0, won = false;
const t0 = Date.now();
while (frames < 60 * 60 * 30) {
  ...
  if (r.goal !== lastGoal) {
    ...
    await page.screenshot({ path: `${out}/run-${style}-${String(r.goal).padStart(2, '0')}.png`, timeout: 90000 });
  }
  if (r.state === 'dead') {
    deaths++;
    ...
    await page.screenshot({ path: `${out}/run-${style}-death.png`, timeout: 90000 });
    if (deaths >= 3) break;
  ...
  if (r.state === 'won') {
    ...
    await page.screenshot({ path: `${out}/run-${style}-win.png`, timeout: 90000 });
    break;
  }
}
