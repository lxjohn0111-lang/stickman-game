...
await page.waitForFunction(() => Math.abs(WT.game.player.yaw - WT.game.level.data.start.yaw) > 0.01, null, { timeout: 10000 }).catch(() => {});
...
await fr.waitForFunction(() => WT.game.state === 'ready', null, { timeout: 300000 });
await fr.click('#lc-start');
